% Succinyl COA synthetase parameter estimation without regulation
...
% Data used for parameter estimations were obtained from Wu et al 2013
...
K_A = 12.5*10^-6; %SCOA binding constant(M)
K_B = 5.7*10^-6; %GDP binding constant(M)
K_C = 2.1*10^-3; %Pi binding constant(M)
K_D = 451*10^-6; %SUC binding constant (M)
K_E = 23.1*10^-6; %GTP binding constant(M)
K_F = 17.7*10^-6; %COA binding constant (M)
Vmaxf = 0.05; %Maximum forward reaction rate(mmol/min)

p0 = [K_A,K_B, K_C,K_D,K_E,K_F Vmaxf];  %Binding Constants initial guess
