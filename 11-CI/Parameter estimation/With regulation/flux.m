...
K_A = 1.0833265e-05; %NADH binding constant estimated Sadek 2004 data (M)
%K_A =  1.7876624e-06; %NADH binding constant estimated Bazil 2010 data (M)
K_B=7.0410629e-05; %UQ binding constant(M)
...
Vmaxf=4.1268454e-03; %Max forward reaction speed   
betaC1=0.3; %CI free energy barrier
dPsi = 0; %mitochondrial membrane potential

%%% metaboloite concentration in the experiment cell
A=Conc(2); %NADH conc.(M)
B=40/10^6; %UQ conc.(M)
C=0; %NAD conc.(M)
D=0; %UQH2 conc.(M)
Ca = Conc(1); %Ca2+ conc.(M)

K_A_Prime=K_A*(1+((n*Ca)/(K_Ca+Ca)));

Vmaxf_Prime=Vmaxf/(1+(Ca/K_Ca));
DeltaGH=F*dPsi+RT*log(Hi/Hm);

dGfor=exp(-betaC1*((4*DeltaGH/RT)+(drG_CI/RT)));
dGrev=exp(-(betaC1-1)*((4*DeltaGH/RT)+(drG_CI/RT)));

deno=(1+A/K_A_Prime+C/K_C)*(1+B/K_B+D/K_D);
J_CI=Vmaxf_Prime/K_A_Prime/K_B*(dGfor*A*B-dGrev*C*D)/deno;
end
