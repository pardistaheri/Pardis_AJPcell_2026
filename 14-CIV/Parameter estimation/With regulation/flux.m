...
drG_CIV=-202.2; %standard Gibs free energy of CIV refrence reaction at T=298.15 K, I=0.1, pH=0 

     
%%% Binding constants for 5 parameters
K_A=2.1564179e-04; %CytCr binding constant 
K_B= 4.3197456e-06; %UQH2 binding constant
K_C =K_A; %CytCo binding constant(M)
K_H1 = mpar(1);
K_H2 = mpar(2);
Vmaxf=mpar(3); %Max forward reaction speed (mmol/min)  
Vmaxf_Prime = Vmaxf/(K_H1/Hi+1+Hi/K_H2);

betaC4=0.3; %CIII free energy barrier
...
J_CIV=Vmaxf_Prime/K_A^2/K_B^0.5*(dGfor*A^2*B^0.5-dGrev*C^2)/deno;
