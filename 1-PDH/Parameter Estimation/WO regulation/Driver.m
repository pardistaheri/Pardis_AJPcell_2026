% Pyruvate Dehydrogenase parameter estimation without regulation
...
% Data used for parameter estimations were obtained from Tsai et al 1973
% Copyright:
...
K_A = 33.2*1e-6; %PYR binding constant in isolated PDH(M)
K_B = 9.9*1e-6; %CoA binding constant(M)
K_C = 60.7*1e-6; %NAD binding constant(M)
Vmaxf = 127.8*1e-6; %Maximum forward reaction rate(mmol/min)

p0 = [K_A, K_B, K_C, Vmaxf];  %Binding Constants initial guess
