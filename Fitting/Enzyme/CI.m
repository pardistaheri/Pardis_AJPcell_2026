...
KH2=1.29e-9; % M 2nd H+
KCa=1.22e-06; % M Ca2+
n=4.77; % unitless Ca2+ coefficient
...
%%% Flux
KA_prime=KA*(1+((n*Cam)/(KCa+Cam)));

Vmaxf_prime=1/(1+(Cam/KCa)+(Hm/KH1)+(KH2/Hm));

deno=(1+A/KA_prime+C/KC)*(1+B/KB+D/KD);
num=1/KA_prime/KB*(exp(-betaC1*(dGr0+4*dGH-RT*log(Hm/1e-7))/RT)*A*B-...
