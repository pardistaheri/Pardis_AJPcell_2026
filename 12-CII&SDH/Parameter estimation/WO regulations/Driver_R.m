% % ComplexII/Succinate dehydregenase parameter estimation reverse reaction
% % % w/o regulation
% SUC + UQ = FUM + UQH2
...
K_B = 1.5187416e-03; %UQH2 mimic binding constant(M)
K_C = 1.80*10^-3; %Fum binding constant(M)
Vmaxf = 1.20^10^-5; %Maximum forward reaction rate(mmol/min)

p0 = [K_B, K_C, Vmaxf];  %Binding Constants initial guess
...
for i = 1:numIters
    ...
    fmincon(@Error_R,p0,[],[],[],[],lbp,ubp,[],options);
