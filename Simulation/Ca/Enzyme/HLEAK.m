...
function T_HLEAK=HLEAK(He,Hm,Cam,dPsi,p)

RT=p.R_con*p.Tem;
%%% Km parameters (Zhang 2018)
KH=1e-7;
KCa=1e-6; % M Ca+ binding constant 
%Concentrations
A=He;
B=Hm;
...
if dPsi>1e-9
 ...
 T_HLEAK=Vmaxf_prime/KH*(A*exp(beta_leak*dPsi.*p.F_con./RT)-B*exp((beta_leak-1)*dPsi.*p.F_con./RT));
